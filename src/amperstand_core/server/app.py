...
async def _capture_worker(stop: asyncio.Event, *, worker_id: int = 0) -> None:
    ...
    loop on httpx/sqlite. Per-job timeout (F2): wraps the thread
...
def create_app(*, docs_visible: bool | None = None) -> FastAPI:
    ...
    def _dispatch(url: str, *, html: str | None, fallback_title: str | None):
        ...
        YouTube, LinkedIn and Reddit have URL-only extractors that fetch
        their canonical content through Anysite (captions, full post text,
        post record). For everything else, prefer the caller-supplied HTML
        when we have it (cheap, uses the user's authenticated browser
        session) and fall back to a fresh server-side fetch.
