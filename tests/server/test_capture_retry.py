...
# ── permanent failures ──────────────────────────────────────────────
#
# An extractor can mark a failure as not worth retrying (Anysite's 412 for a
# post that doesn't exist, a rejected token). Those must not consume retry
# attempts on either path.
...
def _permanent(*_a, **_k):
    ...
    raise AnysiteError("Anysite /api/linkedin/post → 412: Post not found", status=412, retryable=False)
